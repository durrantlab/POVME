...
class FileIO:
    ...
    def load_pdb_into_using_file_object(self, file_obj):
        ...
        the current Molecule object. Note that most users will want to use the
        ...
        for field in (
            self.__parent_molecule.get_constants()["i8_fields"]
        ...
        for field in self.__parent_molecule.get_constants()["i8_fields"]:
            index = self.__parent_molecule.get_atom_information().dtype.names.index(
                field
            )
        ...
        self.__parent_molecule.set_atom_information(
            self.__parent_molecule.numpy_structured_array_remove_field(
                self.__parent_molecule.get_atom_information(), ["empty", "empty2"]
            )
        )
        ...
        self.__parent_molecule.set_coordinates(
            numpy.vstack(
                [
                    ...
                    self.__parent_molecule.get_atom_information()["z"],
                ]
            ).T
        )
        self.__parent_molecule.set_atom_information(
            self.__parent_molecule.numpy_structured_array_remove_field(
                self.__parent_molecule.get_atom_information(), ["x", "y", "z"]
            )
        )  # now remove the coordinates from the atom_information object to save memory

        # string values in
        # self.__parent_molecule.information.get_atom_information() should
        # also be provided in stripped format for easier comparison
    ...
    def select_atoms(self, selection_criteria):
        ...
        try:
            ...
            for key in list(selection_criteria.keys()):
                ...
                if key in self.__parent_molecule.get_constants()["f8_fields"]:
                    vals = [float(v) for v in vals]
                elif key in self.__parent_molecule.get_constants()["i8_fields"]:
            ...
            print(
                "\t"
                + ", ".join(self.__parent_molecule.get_atom_information().dtype.names)
            )
            sys.exit(0)
...
class Molecule:
    ...
    def load_pdb_into(self, filename):
        self.fileio.load_pdb_into(filename)

    def load_pdb_into_using_file_object(self, file_obj):
        self.fileio.load_pdb_into_using_file_object(file_obj)

    # Selections class
    def get_molecule_from_selection(self, selection):
        return self.selections.get_molecule_from_selection(selection)
    ...
    def numpy_structured_array_remove_field(
        self, narray, field_names
...
class ConvexHull:
    ...
    def outside_hull(
        self, our_point, triangles, epsilon=1.0e-5
    ):  # this one used internally
    ...
    def akl_toussaint(self, points):
        ...
        will create an octahedron whose corners are the extremes in x, y, and z
        directions. Every point within this octahedron will be removed because
        ...
        # now check to see if a point is inside or outside the octahedron
        for point in points:
...
# You'll also need a class representing a box of points, with associated
# definitions
...
help_lines.append("")
help_lines.append(
    "--pocket_detection_resolution: The distance between probe points used to initially find the pockets (4.0 by default)."
)
help_lines.append(
    "--pocket_measuring_resolution: The distance between probe points used to measure identified pockets in greater detail. Should divide --pocket_detection_resolution evenly. (1.0 by default)."
)
help_lines.append(
    "--clashing_cutoff: In measuring the pockets, any points closer than this cutoff to receptor atoms will be removed. (3.0 by default)."
)
help_lines.append(
    "--number_of_neighbors: In measuring the pockets, any points with fewer than this number of neighbors will be deleted. These are usually just stray points that don't belong to any real pocket. (4 by default)."
)
help_lines.append("--processors: The number of processors to use. (1 by default).")
help_lines.append(
    "--number_of_spheres: The number of inclusion spheres to generate for each pocket. (5 by default)."
)
help_lines.append(
    "--sphere_padding: How much larger the radius of the inclusion spheres should be, beyond what is required to encompass the identified pockets. (5.0 by default)."
)
help_lines.append("")
help_lines.append("Example:")
help_lines.append("")
help_lines.append(
    "python pocket_id.py --filename rel1_example.pdb --pocket_detection_resolution 4.0 --pocket_measuring_resolution 1.0 --clashing_cutoff 3.0 --number_of_neighbors 4 --processors 1 --number_of_spheres 5 --sphere_padding 5.0 "
)
help_lines.append("")
...
# Get a version of the protein with just the alpha carbons. In my experience,
# that's better for convex hull identification. Otherwise the program
# identifies shallow contors in the protein surface as pockets.
...
# Step 4. Get a box of equispaced points that surround the protein, snapped to
# reso. I'm putting a whole bunch of other functions in this class as well to
# manipulate the points of this box.

printit(
    "Step 4. Making a box of points spaced "
    + str(params["pocket_detection_resolution"])
...
# very fast
box_pts = BoxOfPoints(
    molecule.get_bounding_box(), params["pocket_detection_resolution"] * 4
)

# Step 5. Remove points outside the convex hull. Gradually fill in
# protein-occupying region with denser point fields. Faster this way, I think.
...
printit(
    ...
    + str(params["clashing_cutoff"])
    + " A of any protein atom..."
)
box_pts.remove_all_points_close_to_other_points(
    molecule.get_coordinates(), params["clashing_cutoff"]
)

# Step 7. Now surround each of these points with higher density points that in
# the same regions. This is for getting a more detailed view of the identified
# pockets.
if params["pocket_measuring_resolution"] != params["pocket_detection_resolution"]:
    printit(
        ...
        + str(params["pocket_measuring_resolution"])
        + " A apart for a more detailed measurement of the pocket volume..."
    )
    print("\tAdding points...")
    box_pts.expand_around_existing_points(
        params["pocket_detection_resolution"] / params["pocket_measuring_resolution"],
        params["pocket_measuring_resolution"],
    )
...
# change). Don't know if this is a high pass or low pass filter. I've heard
# these terms, though, and they sound cool.
printit(
    "Step 8. Removing points until all points have at least "
    + str(params["number_of_neighbors"])
    + " neighbors..."
)
box_pts.filter_isolated_points_until_no_change(
    params["pocket_measuring_resolution"], params["number_of_neighbors"]
)
